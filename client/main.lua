...
local function PlayCopIdleAnimation(ped)
    -- تشغيل scenario الوقوف الشرطي
...
local function initializePedModel(model, data)
    CreateThread(function()
        ...
        -- تطبيق الملابس إن وجدت
        if data then
            TriggerEvent('qb-clothing:client:loadPlayerClothing', data, charPed)
        end
        
        -- الانتظار قليلاً ثم تشغيل الأنميشن
        Wait(500)
        PlayCopIdleAnimation(charPed)
    end)
end
...
local function skyCam(bool)
    ...
    if bool then
        ...
        cam = CreateCamWithParams('DEFAULT_SCRIPTED_CAMERA', Config.CamCoords.x, Config.CamCoords.y, Config.CamCoords.z, 5.0, 0.0, Config.CamCoords.w, 20.00, false, 0)
...
RegisterNUICallback('cDataPed', function(nData, cb)
    ...
    if cData ~= nil then
        QBCore.Functions.TriggerCallback('qb-multicharacter:server:getSkin', function(skinData)
            if skinData then
                -- ======================================================
                -- الشخصية المحفوظة مع skin (illenium-appearance)
                -- ======================================================
                ...
                CreateThread(function()
                    ...
                    -- الانتظار حتى يكتمل تطبيق المظهر ثم تشغيل الأنميشن
                    Wait(800)
                    TaskStartScenarioInPlace(charPed, "WORLD_HUMAN_GUARD_STAND", 0, true)
                end)
            else
                -- ======================================================
                -- الشخصية المحفوظة بدون skin (random model)
                -- ======================================================
                CreateThread(function()
                    local randommodels = {
                        "mp_m_freemode_01",
...
RegisterNUICallback('updateCreationPed', function(data, cb)
    local model = joaat(data.model or 'mp_m_freemode_01')
    ...
    -- إنشاء الشخصية الجديدة بالموديل المطلوب
    CreateThread(function()
        RequestModel(model)
        while not HasModelLoaded(model) do
            Wait(0)
